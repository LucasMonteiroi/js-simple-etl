...
class WorkerLoader { 
  ...
  async runFromSqlToSql(etl, useBearerAuth = false) {
    return { status: 'Running', description: 'Running from sql to sql', data: etl, useBearerToken: useBearerAuth, percent: 40 };
  }

  async runFromSqlToApi(etl, useBearerAuth = false) {
    return { status: 'Running', description: 'Running from sql to rest', data: etl, useBearerToken: useBearerAuth, percent: 40 };
  }

  async runFromApiToSql(etl, useBearerAuth = false) {
    return { status: 'Running', description: 'Running from rest to sql', data: etl, useBearerToken: useBearerAuth, percent: 40 };
  }

  async runFromApiToApi(etl, useBearerAuth = false) {
    return { status: 'Running', description: 'Running from rest to rest', data: etl, useBearerToken: useBearerAuth, percent: 40 };
  }
